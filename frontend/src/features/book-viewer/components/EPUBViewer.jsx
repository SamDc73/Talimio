...
const EPUBViewer = ({ url, bookInfo }) => {
  ...
  const onRendition = (rendition) => {
    ...
    // Load saved location
    if (bookInfo?.id) {
      const savedLocation = localStorage.getItem(`epub-location-${bookInfo.id}`);
      if (savedLocation) {
    ...
    <div className="epub-viewer-container">
      <div className="epub-controls-bar">
        <div className="epub-controls">
          <button type="button" onClick={() => setShowToc(!showToc)} className="epub-button">
            {showToc ? 'Hide' : 'Show'} Table of Contents
          </button>
          <button type="button" onClick={decreaseFontSize} className="epub-button">
            A-
          </button>
          <span className="font-size-display">{size}%</span>
          <button type="button" onClick={increaseFontSize} className="epub-button">
            A+
          </button>
        </div>
      </div>

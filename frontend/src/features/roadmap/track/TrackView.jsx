...
export default function TrackView({ roadmapId }) {
  ...
  }, [initialPositioningDone, modules]); // Add modules as dependency to trigger recalculation when data changes
  ...
  if (error) {
    return (
      <div className="flex-1 min-h-screen flex flex-col items-center justify-center text-red-600">
        <p>Error loading track view: {error}</p>
      </div>
    );
  }

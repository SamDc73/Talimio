...
function OutlineItem({ module, index, onLessonClick, isLessonCompleted, toggleLessonCompletion, courseProgress }) {
  ...
  const isItemCompleted = (item) => {
    return isLessonCompleted?.(item.id) || item.status === "completed" || item.status === "done";
  };
  ...
  const LessonContent = ({ lesson, isCompleted, currentLessonIndexStr, idx, onLessonClick }) => {
    return (
      <div
        className={cn(
  ...
  const LessonItem = ({ lesson, idx, depth, parentIndexStr, onLessonClick }) => {
    ...
    const depthClass = depth > 0 ? "ml-6 mt-3 border-l-2 border-emerald-100/50 pl-4 pt-3" : "mt-3";
      ...
      >
        <div
          className={cn(
            ...
            isModuleCompleted ? "bg-emerald-500 text-white hover:bg-emerald-600" : "bg-zinc-100 text-zinc-700"
          )}
        >

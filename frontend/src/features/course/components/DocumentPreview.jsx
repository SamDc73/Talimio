...
const DocumentPreviewModal = ({ 
      ...
      setTimeout(() => {
        if (document.document_type === 'url') {
          ...
          setContentPreview({
            ...
            thumbnails: Array.from({ length: 5 }, (_, i) => `/api/v1/documents/${document.id}/pages/${i + 1}/thumbnail`)
          });
        }
    ...
    const handleKeydown = (e) => {
      ...
      switch (e.key) {
        ...
        case 'ArrowLeft':
          ...
          break;
        case 'ArrowRight':
          if (contentPreview?.type === 'pdf' && currentPage < (contentPreview.totalPages || 1)) {
            setCurrentPage(prev => prev + 1);
          }
          break;
        case '+':
  ...
  }, [isOpen, currentPage, contentPreview, onClose, document.addEventListener, document.removeEventListener]);
            ...
            {contentPreview?.type === 'pdf' && (
              <>
                ...
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => setCurrentPage(prev => Math.min(prev + 1, contentPreview.totalPages))}
                ...
                <div className="p-4">
                  ...
                  <dl className="space-y-2 text-sm">
                    <div>
                      ...
                      <dd className="text-gray-900 capitalize">{document.document_type}</dd>
                    </div>
                    <div>
                      <dt className="font-medium text-gray-600">Status:</dt>
                      <dd><DocumentStatusBadge status={document.status} size="sm" /></dd>
                    </div>
                    {document.size && (
                      <div>
                    ...
                    <div>
                      ...
                      <dd className="text-gray-900">{formatDate(document.created_at)}</dd>
                    </div>
                    {document.processed_at && (
                      <div>
                        <dt className="font-medium text-gray-600">Processed:</dt>
                        <dd className="text-gray-900">{formatDate(document.processed_at)}</dd>
                      </div>
                    )}
                  </dl>
                </div>

...
import { Upload, Link2, X, FileText, AlertCircle, CheckCircle2 } from 'lucide-react';
...
const DocumentUploader = ({ 
  ...
  const handleAddUrl = () => {
    if (!urlInput.trim() || !urlTitle.trim() || disabled) return;
  ...
  return (
    <div className="space-y-4">
      ...
      {showPreview && documents.length > 0 && (
        <Card ref={documentsRef} className="border-green-200/60 bg-gradient-to-r from-green-50 to-emerald-50 dark:from-green-900/10 dark:to-emerald-900/10 shadow-sm">
          <div className="p-5">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="text-xs text-green-600/80 dark:text-green-400/80 font-medium">
                {(documents.reduce((acc, doc) => acc + (doc.size || 0), 0) / 1024 / 1024).toFixed(1)}MB total
              </div>
            </div>
      ...
      <Card className={`border-dashed ${dragActive ? 'border-blue-500 bg-blue-50' : 'border-gray-300'} ${documents.length > 0 ? 'border-gray-200' : ''}`}>
        <div
          className={`${documents.length > 0 ? 'p-4' : 'p-6'} text-center cursor-pointer transition-colors ${
        ...
        <div className={documents.length > 0 ? 'p-3' : 'p-4'}>
          ...
          <div className="space-y-2">
            ...
            <Button
              ...
              disabled={disabled || !urlInput.trim() || !urlTitle.trim()}
              size="sm"
              className="w-full"
            >

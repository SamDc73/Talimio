...
import { calculateBookProgress, getBookProgressStats } from "@/services/tocProgressService";
...
const BaseCard = ({ item, pinned, onTogglePin, onDelete, onArchive, onTagsUpdated, index, onClick }) => {
  ...
  const videoProgress = useAppStore((state) => (item.type === "youtube" ? state.videos.progress[item.uuid] : null));
  ...
  const progressValue = isFlashcard
    ? item.totalCards > 0
      ? ((item.totalCards - (item.due || 0) - (item.overdue || 0)) / item.totalCards) * 100
      : 0
    : isBook
    ? (() => {
  ...
  const handleConfirmDelete = async () => {
    try {
      const itemType = item.type === "youtube" ? "video" : item.type === "flashcards" ? "flashcard" : item.type;
  ...
  return (
    <div>
        ...
        <div className="p-6 flex flex-col justify-between h-full">
          ...
          {item.type === "youtube" && (
            <p className="text-muted-foreground text-sm mb-4">
              by {item.channelName || item.channel || "Unknown Channel"} • {formatDuration(item.duration)}
            </p>
          )}
      ...
      <TagEditModal
        ...
        contentType={item.type === "youtube" ? "video" : item.type === "flashcards" ? "flashcard" : item.type}
...
export default function HomePage() {
  ...
  useEffect(() => {
    async function loadContentData() {
      ...
      try {
          ...
          content.map(async (item) => {
            ...
            try {
              ...
              if (contentId && contentType) {
                const response = await fetch(`/api/tags/${contentType}/${contentId}/tags`);
                if (response.ok) {
        ...
        const courseItems = transformedContent.filter((item) => item.type === "course");
        if (courseItems.length > 0) {
          ...
          const contentWithProgress = transformedContent.map((item) => {
            if (item.type === "course") {
              const courseWithProgress = coursesWithProgress.find((c) => c.id === item.id);
  ...
  const handleArchiveItem = async (itemId, contentType, newArchivedState) => {
    ...
    try {
      // Include archived content if we're showing archived or all content
      const includeArchived = archiveFilter === "archived" || archiveFilter === "all";
      console.log(`📡 Refreshing content with includeArchived: ${includeArchived}`);
        ...
        <ErrorBoundary>
          <div className="min-h-screen bg-gradient-to-b from-slate-50 to-slate-100">
            ...
            <div className="fixed bottom-8 right-8 z-50">
              ...
              <Button className="rounded-full h-16 w-16 shadow-lg" onClick={() => setIsFabExpanded(!isFabExpanded)}>
            ...
            >
              <SheetContent side="bottom" className="sm:max-w-lg mx-auto">
                ...
                <SheetHeader>
                  ...
                  <SheetDescription>
                    Upload a PDF, EPUB, or other supported document to start learning.
                  </SheetDescription>
                </SheetHeader>
                <div className="py-4 space-y-4">
                  <div className="grid w-full max-w-sm items-center gap-1.5">
                    <Label htmlFor="book-file">Book File</Label>
                    <Input id="book-file" type="file" accept=".pdf,.epub,.mobi,.azw3" onChange={handleFileChange} />
                  </div>
                ...
                <SheetFooter>
                  <Button onClick={handleUpload} disabled={!selectedFile || !bookTitle.trim() || !bookAuthor.trim()}>
                    Upload Book
                  </Button>
                </SheetFooter>
              </SheetContent>
            ...
            >
              <SheetContent side="bottom" className="sm:max-w-lg mx-auto">
                ...
                <SheetHeader>
                  ...
                  <SheetDescription>Enter a YouTube URL to add it to your library.</SheetDescription>
                </SheetHeader>
                <div className="py-4">
                  <Input
                    type="url"
                ...
                <SheetFooter>
                  <Button onClick={handleYoutubeAdd} disabled={!youtubeUrl.trim() || isAddingVideo}>
            ...
            <Sheet open={showFlashcardDialog} onOpenChange={setShowFlashcardDialog}>
              <SheetContent side="bottom" className="sm:max-w-lg mx-auto">
                <SheetHeader>
                  ...
                  <SheetDescription>Create a new deck of flashcards. You can add cards now or later.</SheetDescription>
                </SheetHeader>
                <div className="py-4 space-y-4">
                  <Input

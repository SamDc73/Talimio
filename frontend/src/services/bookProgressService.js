 ...
 * @param {Object} book - Book object with table_of_contents
...
export function calculateBookProgress(book) {
  ...
  const tocProgressKey = `book_toc_progress_${book.id}`;
  ...
  if (savedTocProgress) {
    try {
      ...
      savedIds.forEach(id => completedSectionIds.add(id));
    } catch (e) {
  ...
  if (book.table_of_contents && book.table_of_contents.length > 0) {
    totalSections = countAllSections(book.table_of_contents);
    completedSections = countCompletedSections(book.table_of_contents);
  }
...
export function updateBookProgress(bookId, sectionId, isCompleted) {
  const tocProgressKey = `book_toc_progress_${bookId}`;
  const savedTocProgress = localStorage.getItem(tocProgressKey);
  const completedSectionIds = new Set();
  ...
  const tocProgress = Array.from(completedSectionIds).filter(id => 
  ...
  window.dispatchEvent(new CustomEvent('bookProgressUpdate', {
    detail: { bookId }
  }));
}
...
/**
 * Get quick progress stats for a book (when table_of_contents not available)
...
export function getBookProgressStats(bookId) {
  // Get the saved stats
  const statsKey = `book_progress_stats_${bookId}`;
 ...
 */
export function saveBookProgressStats(bookId, totalSections, completedCount = null) {
  const statsKey = `book_progress_stats_${bookId}`;
  
  // If completedCount not provided, count it ourselves
  const finalCompletedCount = completedCount !== null ? completedCount : getCompletedSections(bookId).size;
  
  const percentage = totalSections > 0 ? Math.round((finalCompletedCount / totalSections) * 100) : 0;
...
if (typeof window !== 'undefined') {
  ...
  allKeys.forEach(key => {
    if (key.startsWith('book_progress_stats_')) {
      try {

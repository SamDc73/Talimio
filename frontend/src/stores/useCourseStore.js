		...
		persist(
			immer((set, get) => ({
				...
				getLessonStatus: (moduleId, lessonId) => {
					return (
						get().lessons.status[moduleId]?.[lessonId] || DEFAULT_LESSON_STATUS
					);
				},
				...
				recalculateModuleProgress: (courseId, moduleId) => {
					...
					const moduleProgressData =
						get().progress.lessons[courseId]?.[moduleId] || {};
					...
					if (typeof window !== "undefined") {
						window.dispatchEvent(
							new CustomEvent("courseProgressUpdate", {
								detail: {
									courseId,
									progress: get().progress.courses[courseId],
								},
							}),
						);
					}
				...
				getModuleProgress: (courseId, moduleId) => {
					return (
						get().progress.modules[courseId]?.[moduleId] || {
							totalLessons: 0,
							completedLessons: 0,
							inProgressLessons: 0,
							completionPercentage: 0,
...
export const selectCourseProgress = (courseId) => (state) =>
	state.getCourseProgress(courseId);
export const selectModules = (courseId) => (state) =>
	state.getModules(courseId);
export const selectLessons = (moduleId) => (state) =>
	state.getLessons(moduleId);

...
const checkReactVersion = () => {
	...
	if (reactVersion.startsWith("19")) {
		if (import.meta.env.VITE_DEBUG_MODE === "true") {
			...
			const experimentalFeatures = Object.keys(React).filter(
				(key) => key.startsWith("experimental") || key.startsWith("use"),
			);
			console.log(
				"[Debug] Available experimental features:",
...
const checkRadixComponents = async () => {
	try {
		...
		if (import.meta.env.VITE_DEBUG_MODE === "true") {
			console.log(
				"[Debug] Tooltip component loaded:",
		...
		if (import.meta.env.VITE_DEBUG_MODE === "true") {
			console.log(
				"[Debug] RadioGroup component loaded:",
		...
		if (import.meta.env.VITE_DEBUG_MODE === "true") {
			console.log(
				"[Debug] Separator component loaded:",

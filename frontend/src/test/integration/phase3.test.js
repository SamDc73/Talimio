...
describe('Phase 3 Integration Tests', () => {
  ...
  describe('Book Chapter Management', () => {
    ...
    it('should extract book chapters when requested', async () => {
      ...
      const TestExtractComponent = ({ bookId }) => {
        const extractApi = useApi('/books/{bookId}/extract-chapters', { method: 'POST' })
        ...
        return (
          <button onClick={handleExtract} data-testid="extract-button">
            Extract Chapters
          </button>
        )
      }
    ...
    it('should update chapter status when marked complete', async () => {
      ...
      const TestChapterComponent = ({ bookId, chapterId }) => {
        const statusApi = useApi('/books/{bookId}/chapters/{chapterId}/status', { method: 'PUT' })
        ...
        return (
          <button onClick={handleComplete} data-testid="complete-chapter">
            Complete Chapter
          </button>
        )
      }
  ...
  describe('Video Chapter Management', () => {
    it('should fetch and display video chapters from new API', async () => {
      ...
      expect(screen.getByText(/Main Content - not_started/)).toBeInTheDocument()
    })
    ...
    it('should extract video chapters when requested', async () => {
      ...
      const TestVideoExtractComponent = ({ videoUuid }) => {
        const extractApi = useApi('/videos/{videoUuid}/extract-chapters', { method: 'POST' })
        ...
        return (
          <button onClick={handleExtract} data-testid="extract-video-chapters">
  ...
  describe('Progress Service Integration', () => {
    ...
    it('should handle progress cache correctly', async () => {
      const { getNodeProgress, _progressCache } = await import('@/services/progressService')
  ...
  describe('Error Handling and Loading States', () => {
    ...
    it('should handle and display API errors', async () => {
      ...
      const TestErrorComponent = () => {
        ...
        return (
          <div>
            {api.error && <div data-testid="error">Error: {api.error.message}</div>}
            {api.data && <div data-testid="data">Data loaded</div>}
          </div>
        )
      }
        ...
        return (
          <div>
            <div data-testid="progress">
              Progress: {roadmapApi.data?.filter(n => n.status === 'completed').length} / {roadmapApi.data?.length}
            </div>

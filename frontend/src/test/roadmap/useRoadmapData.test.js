...
describe('useRoadmapData Hook', () => {
  ...
  it('should fetch roadmap data successfully', async () => {
    ...
    expect(result.current.error).toBeNull()
    expect(fetch).toHaveBeenCalledWith('http://localhost:8080/api/v1/roadmaps/roadmap-123')
  })
  ...
  it('should refetch when roadmapId changes', async () => {
    ...
    expect(fetch).toHaveBeenNthCalledWith(1, 'http://localhost:8080/api/v1/roadmaps/roadmap-1')
    expect(fetch).toHaveBeenNthCalledWith(2, 'http://localhost:8080/api/v1/roadmaps/roadmap-2')
  })
  ...
  describe('Phase 3 Integration', () => {
    it('should use new roadmap nodes endpoint', async () => {
      ...
      const mockNodes = [
        { id: 'node-1', title: 'Node 1', status: 'completed', completion_percentage: 100 },
        { id: 'node-2', title: 'Node 2', status: 'in_progress', completion_percentage: 50 },
        { id: 'node-3', title: 'Node 3', status: 'not_started', completion_percentage: 0 }
      ]

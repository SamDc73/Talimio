...
Thanks for watching!
  ...
  it('renders video sidebar with basic info', () => {
    render(<VideoSidebar video={mockVideo} currentTime={0} onSeek={mockOnSeek} />)
  ...
  it('highlights active chapter based on current time', () => {
    render(<VideoSidebar video={mockVideo} currentTime={400} onSeek={mockOnSeek} />)
  ...
  it('shows fallback when no chapters found', () => {
    ...
    render(<VideoSidebar video={videoWithoutChapters} currentTime={0} onSeek={mockOnSeek} />)

    expect(screen.getByText('No chapters found')).toBeInTheDocument()
    expect(screen.getByText('Chapters are extracted from the video description')).toBeInTheDocument()
  })

  it('returns null when no video is provided', () => {
    const { container } = render(<VideoSidebar video={null} currentTime={0} onSeek={mockOnSeek} />)
    expect(container.firstChild).toBeNull()
  })

  it('formats duration correctly', () => {
    const shortVideo = { ...mockVideo, duration: 125 } // 2 minutes 5 seconds
    render(<VideoSidebar video={shortVideo} currentTime={0} onSeek={mockOnSeek} />)
...
1:23:45 Third chapter with hours
      ...
      render(<VideoSidebar video={videoWithVariousFormats} currentTime={0} onSeek={mockOnSeek} />)
...
5:00 *** Getting Started ***
      ...
      render(<VideoSidebar video={videoWithMessyTitles} currentTime={0} onSeek={mockOnSeek} />)
  ...
  describe('Phase 3 Chapter Integration', () => {
    ...
    it('should extract chapters when button is clicked', async () => {
      const { useApi } = await import('@/hooks/useApi')
      const mockExtract = vi.fn().mockResolvedValue({ message: 'Chapters extracted', count: 6 })

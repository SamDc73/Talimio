...
@router.get("/{deck_id}")
async def get_deck_endpoint(deck_id: UUID) -> FlashcardDeckResponse:
    """Get deck details."""
    return await get_deck(deck_id)


@router.post("", status_code=status.HTTP_201_CREATED)
async def create_deck_endpoint(deck_data: FlashcardDeckCreate) -> FlashcardDeckResponse:
    """Create a new deck."""
    return await create_deck(deck_data)


@router.put("/{deck_id}")
async def update_deck_endpoint(deck_id: UUID, deck_data: FlashcardDeckUpdate) -> FlashcardDeckResponse:
    """Update deck details."""
...
async def create_card_endpoint(deck_id: UUID, card_data: FlashcardCardCreate) -> FlashcardCardResponse:

...
class GeneratedTag(TypedDict):
    """Tag generated by the LLM with normalized confidence."""

    tag: str
    confidence: float


class BatchTagItem(TypedDict, total=False):
    """Batch tag request item accepted by the service boundary."""

    content_id: ReadOnly[JsonValue]
    user_id: ReadOnly[JsonValue]
...
class TaggingService:
    ...
    async def _generate_tags_llm(self, title: str, content_preview: str) -> list[GeneratedTag]:
    ...
    async def batch_tag_content(self, content_items: list[BatchTagItem]) -> dict[str, object]:

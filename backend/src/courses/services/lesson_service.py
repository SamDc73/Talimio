...
class LessonService:
    ...
    def _build_generation_mode_context(
        ...
        *,
        ...
        if recommendation is not None:
            lines.append(f"Adaptive Recommendation: {recommendation.action}")
    ...
    async def _build_generation_context_sections(  # noqa: PLR0912
        ...
        *,
        ...
        if stable_objective_context:
            context_sections.append(stable_objective_context)

        previous_lesson_context = self._build_previous_lesson_context(current_version=current_version)
        if previous_lesson_context:
            context_sections.append(previous_lesson_context)

        feedback_context = await self._build_feedback_context(course_id=course.id)
        if feedback_context:
            context_sections.append(feedback_context)

        if immediate_regenerate_request:
            context_sections.append(f"## Regeneration Request\n{immediate_regenerate_request}")

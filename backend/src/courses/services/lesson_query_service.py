...
class LessonQueryService:
    ...
    async def get_lesson(  # noqa: C901
        ...
        Args:
            ...
            user_id: User ID for ownership check (required for generation)
        ...
        if user_id:
            query = query.where(Roadmap.user_id == user_id)
        ...
        if generate and (not lesson.content or len(lesson.content.strip()) == 0):
            ...
            try:
                ...
                auth = AuthContext(user_id, self.session)
    ...
    async def get_lesson_simplified(
        ...
        return await self.get_lesson(course_id, lesson_id, generate, user_id)

...
async def list_servers(
    ...
    servers, total = await paginate_user_mcp_servers(db, user_id, page=page, page_size=page_size)
    return {
...
async def create_server(
    ...
    try:
        server = await create_user_mcp_server(db, user_id=user_id, payload=payload)
...
async def delete_server(
    ...
    deleted = await delete_user_mcp_server(db, user_id=user_id, server_id=server_id)
    if not deleted:

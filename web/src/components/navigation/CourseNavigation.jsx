...
function calculateLessonNavigation(modules, currentModuleId, currentLessonId) {
	...
	const currentIndex = allLessons.findIndex(
		(lesson) => lesson.moduleId === currentModuleId && lesson.id === currentLessonId
	)
				...
				{navigation.previous ? (
					...
					>
						...
						<div className="text-left truncate">
							...
							<div className="font-medium truncate">{navigation.previous.title || navigation.previous.slug}</div>
						</div>
					</Button>
				) : (
				...
				{navigation.next ? (
					...
					>
						<div className="text-right truncate">
							...
							<div className="font-medium truncate">{navigation.next.title || navigation.next.slug}</div>
						</div>
				...
				<div key={module.id} className="border border-gray-200 dark:border-gray-700 rounded-lg">
					...
					<button
						...
						className={`w-full text-left p-3 rounded-t-lg border-b border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors ${
							currentModuleId === module.id ? "bg-blue-50 dark:bg-blue-900/20 border-blue-200 dark:border-blue-800" : ""
						}`}
					>
						<div className="flex items-center justify-between">
							<div className="flex-1">
								<div className="font-medium text-gray-900 dark:text-gray-100">{module.title}</div>
								{module.description && (
									<div className="text-sm text-gray-600 dark:text-gray-400 mt-1">{module.description}</div>
								)}
							</div>
							{module.status === "completed" && <CheckCircle className="h-5 w-5 text-green-500" />}
						</div>
					</button>
					...
					{module.lessons && module.lessons.length > 0 && (
						<div className="p-2 space-y-1">
							{module.lessons.map((lesson, _lessonIndex) => (
								...
								>
									<div className="flex items-center justify-between">
										<span className="truncate">{lesson.title || lesson.slug}</span>

...
export function LessonViewer({
	...
	if (lesson.generationStatus === "failed") {
		return (
			<div className="h-[calc(100vh-4rem)] overflow-y-auto w-full">
				<div className="flex flex-col items-center justify-center min-h-full text-destructive">
					<div className="mx-auto max-w-container-3xl rounded-2xl border border-destructive/30 bg-destructive/10 p-6 shadow-sm">
						<h2 className="text-xl font-semibold mb-4">We couldn't generate this lesson</h2>
						<p className="mb-4">{lesson.generationError || "Please try opening it again in a moment."}</p>
						<Button onClick={onBack} variant="outline" size="sm">
							<ArrowLeft className="size-4 " />
							Back
						</Button>
	...
	// Content is still being generated by the durable worker; the hook polls until ready.
	if (lesson.generationStatus === "generating") {
		return (
			<div className="h-[calc(100vh-4rem)] overflow-y-auto w-full">
				<div className="flex flex-col items-center justify-center min-h-full text-muted-foreground">
					<div className="animate-spin rounded-full size-12 border-y-2 border-primary mb-4" />
					<p className="font-medium text-foreground">Writing this lesson</p>
					<p className="text-sm">This usually takes a few seconds. It will appear automatically.</p>
				</div>
			</div>
		)
	}

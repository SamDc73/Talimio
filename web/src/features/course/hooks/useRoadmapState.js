...
export const useRoadmapState = (roadmapId, onError) => {
	...
	const updateRoadmapState = (roadmap) => {
		setRoadmap(roadmap)
	...
	const initializeRoadmap = async (roadmapId) => {
		...
		setIsLoading(true)
		try {
			const response = await fetch(`${import.meta.env.VITE_API_BASE || "/api/v1"}/courses/${roadmapId}?generate=true`)
			if (!response.ok) {
	...
	useEffect(() => {
		...
		initializeRoadmap(roadmapId).catch((_error) => {
			onError?.()
		})
	}, [roadmapId, onError, initializeRoadmap, setEdges, setNodes])

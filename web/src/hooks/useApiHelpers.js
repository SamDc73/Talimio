...
export function useNodeApi(nodeId = null) {
	...
	return {
		...
		// Loading states
		isLoading:
			getNode.isLoading || updateNode.isLoading || updateStatus.isLoading,
...
export function useBookChaptersApi(bookId = null) {
	...
	return {
		...
		async fetchChapter(chapterId) {
			if (!bookId || !chapterId)
				throw new Error("Book ID and Chapter ID required");
		...
		// Loading states
		isLoading:
			getChapters.isLoading ||
			getChapter.isLoading ||
			updateChapterStatus.isLoading ||
			extractChapters.isLoading,
		error:
			getChapters.error ||
			getChapter.error ||
			updateChapterStatus.error ||
